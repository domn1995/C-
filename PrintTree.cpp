...
void PrintMemInfo(TreeNode* node)
{
	...
	if (node->nodeKind == DeclK && node->kind.decl == ParamK || node->isParam)
	{		
		printf("Param, ");		
	}
	else if (node->nodeKind == ExpK && node->kind.exp == CallK || node->nodeKind == StmtK && node->kind.stmt == CompK || node->expType == Undefined)	
	{
		printf("None, ");
	}
